...
def gen_multi_grids_getstats():
    ...
    for loop in range(number_to_loop):
        ...
        #if loop != 0: continue  # ******TEST ONLY GET RID OF THIS!!
        ...
        if success == "success": success3 += 1

        #v4=single location checker
        start_time = time()
        gridgen.create_blank_grids(values_only=True)
        success,iterations = gridgen.new_iterate(debug=False,timeout=common_timeout,always_wholegrid_least=True,single_location_checker=True)
        elapsed = round(time() - start_time, 2)
        legit = gridgen.is_grid_legit()
        ...
        if success == "success": success5 += 1

        #v5=single location checker/not wholegrid
        start_time = time()
        gridgen.create_blank_grids(values_only=True)
        success,iterations = gridgen.new_iterate(timeout=common_timeout,single_location_checker=True,always_wholegrid_least="hybrid")
        elapsed = round(time() - start_time, 2)
        legit = gridgen.is_grid_legit()
        print(f"#{loop} NEW -locationchecker-hybrid  cells iterated {iterations:,}  success? {success},  legit? {legit}   time {elapsed}")
        # print()
        # print(gridgen.grid)
        timing6.append(elapsed)
        if success!="timed out": results6+=1
        if success == "success": success6 += 1
    ...
    print ("original",np.average(timing1),results1,success1)
    print("new method", np.average(timing2),results2,success2)
    print("new method -- wider", np.average(timing3), results3,success3)
    print("new method -- singlelocation", np.average(timing4), results4,success4)
    print("new method -- singlelocation-notwide", np.average(timing5), results5,success5)
    print("new method -- singlelocation-hybrid", np.average(timing6), results6,success6)

def quick_or_slow_test():
    gridgen.initialise_grid(rows=10,cols=13)
    ...
    for speed in speeds:
        ...
        while True:
            ...
            result,iterations=gridgen.new_iterate(speed,single_location_checker=True,always_wholegrid_least=False)
